...
contract BCDCVault is SafeMath {
    ...
    /// @notice Constructor function sets the BCDC Multisig address and
    /// total number of locked tokens to transfer
    function BCDCVault(address _bcdcMultisig) internal {
    ...
    /// @notice Transfer Development Team Tokens To MultiSigWallet - 30 Days Locked
    ...
    /// @notice Transfer Founders Team Tokens To MultiSigWallet - 365 Days Locked
    function unlockForDevelopment() external {
        /// If it has not reached 365 days mark do not transfer
        if (block.number < unlockedAllTokensForFounders) throw;
        /// If it is already unlocked then do not allowed
